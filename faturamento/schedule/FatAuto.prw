	...
	BeginSql alias 'TRB'
		...
		SELECT PEDIDO FROM
			(SELECT * FROM
				...
				WHERE SC5.%notDel% AND C5_FILIAL = %xfilial:SC5% AND C5_NOTA = ' ' AND C5_BLQ = ' ' AND C5_X_BLQ NOT IN ('S','C') AND C5_X_BLQFI <> 'S' AND NOT (COALESCE(SC9.C9_ITEM,'SL') = 'SL' AND A1_X_FATPA = 'S')) PED_A_FATURAR
