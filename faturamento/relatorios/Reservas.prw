...
Static Function GeraArqTRB()
	...
	BeginSql alias 'TRB'
		...
		SELECT C0_PRODUTO,B1_DESC,C0_LOCAL,C0_DOCRES,C0_QTDORIG,C0_QUANT, CASE WHEN C0_QTDORIG = C0_QUANT THEN 'FATURADO' ELSE 'PARCIAL' END CONSUMO, C5_NUM PEDIDO FROM %table:SC5% SC5
		INNER JOIN %table:SC0% SC0 ON C5_FILIAL = C0_FILIAL AND C5_NUM = C0_DOCRES AND C0_QUANT > 0 AND C0_SOLICIT = '' AND SC0.%notDel%
		...
		WHERE SC0.%notDel% AND C0_FILIAL = %xfilial:SC0% AND C0_QUANT > 0 AND C0_DOCRES <> ' ' AND C0_SOLICIT = '' AND C5_NUM IS NULL
		ORDER BY C0_PRODUTO
