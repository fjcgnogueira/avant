...
‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±…ÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕª±±
±±∫Programa  ≥ Fat_Canal()  ∫ Autor ≥ Fernando Nogueira ∫ Data ≥ 18/06/15 ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕπ±±
...
User Function Fat_Canal(aParam)
	...
	//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
	//≥aParam     |  [01]  | [02]  |  [03]   |   [04]   |  [05]  |  [06] |  [07]    |  [08]   |  [09]  |
	//≥           | Canal  | Grupo | Data De | Data Ate | E-mail |   CC  | Schedule | Empresa | Filial |
	//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
	...
	*/
	...
	Private _cTitulo    := "Faturamento Di·rio Canal"
	...
	If Empty(aParam[3])
		_lReturn := .F.
		If _lSchedule
			Conout("Par‚metro 3 È ObrigatÛrio!")
		Else
			MsgInfo("Par‚metro 3 È ObrigatÛrio!")
		Endif
	...
	ElseIf ValType(aParam[3]) == "C" .And. aParam[3] == "ACUMULADO"
		...
		If !_lReturn
			If _lSchedule
				Conout("ProgramaÁ„o n„o cadastrada!")
			Else
				MsgInfo("ProgramaÁ„o n„o cadastrada!")
			Endif
		Endif
...
±±∫Programa  ≥GeraArqTRB∫Autor  ≥ Fernando Nogueira  ∫ Data ≥ 09/01/2014  ∫±±
...
If TRB->(!Eof())
	...
	ElseIf SX5->(dbSeek(xFilial("SX5")+"T3"+_cCanal))
		...
		While TRA->(!Eof())
			...
			aAdd((oHTML:ValByName("aRg.cRegiao")), TRA->Regiao)
			aAdd((oHTML:ValByName("aRg.cQtd"))   , Transform(TRA->QTD               , _cPctQtd ))
			aAdd((oHTML:ValByName("aRg.cQtdPrc")), Transform(TRA->QTD/_nTotReg*100  , _cD2Total)+'%')
			aAdd((oHTML:ValByName("aRg.cVlr"))   , Transform(TRA->Reais             , _cD2Total))
			aAdd((oHTML:ValByName("aRg.cVlrPrc")), Transform(TRA->Reais/_nTotVlr*100, _cD2Total)+'%')
			
			TRA->(dbSkip())
					
		EndDo

		oHtml:ValByName("cQtdRg", Transform(_nTotReg, _cPctQtd))
		oHtml:ValByName("cVlrRg", Transform(_nTotVlr, _cD2Total))
		...
		oHtml:ValByName("cQtdReg", Transform(_nTotQtd, _cPctQtd))
		oHtml:ValByName("cVlrReg", Transform(_nTotReal, _cD2Total))
		...
		oHtml:ValByName("cQtdGrp", Transform(_nTotQtd, _cPctQtd))
		oHtml:ValByName("cVlrGrp", Transform(_nTotReal, _cD2Total))
	...
	If _cCanal <> 'VENDEDORES'
		...
		If Empty(_cEmail)
			...
			Else
				oProcess:cTo  := SepEmail(_cString,_cCanal)
				If lAcumulado .And. _cCanal = 'CN0002'
