...
If SC5->(AllTrim(C5_X_BLQ)+AllTrim(C5_X_BLQFI)+AllTrim(C5_LIBEROK)) == 'SSS'
	...
	If MsgYesNo("Pedido "+SC5->C5_NUM+" Estแ com Bloqueio Fiscal!"+Chr(13)+Chr(10)+"Liberar do Bloqueio Avant assim mesmo?")
		SC5->(RecLock("SC5",.F.))
			SC5->C5_X_BLQ := 'N'
		SC5->(MsUnlock())
...
ElseIf SC5->(AllTrim(C5_X_BLQ)) $ 'CS'
	...
	If TRB->(Eof())
		...
		Endif
	Else
		ApMsgInfo("ษ necessแrio tirar a Libera็ใo Parcial do Pedido "+SC5->C5_NUM+" para fazer a Libera็ใo Avant!")
	Endif
