...
Static Function GeraArqTRB()
	...
	BeginSql alias 'TRB'
		...
		SELECT CLIENTE,LOJA,COND_CLI,COND_PED FROM
			...
			WHERE C5_FILIAL = %exp:xFilial("SC5")% AND C5_NOTA = ' ' AND C5_BLQ = ' ' AND C5_X_BLQ NOT IN ('S','C') AND C5_X_BLQFI <> 'S' AND C5_X_BLFIN <> 'S' AND SC5.%notDel% AND C5_CLIENTE = %exp:_cCliente% AND C5_LOJACLI = %exp:_cLoja%
