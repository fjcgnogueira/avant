...
Static Function GeraArqTRB()
	...
	BeginSql alias 'TRB'
		...
		SELECT VENDEDOR, NOME, CREDITO, DEBITO, SALDO, A3_XNOMID NOMID, A3_ACMMKT ACUMULADOR FROM
		...
		(SELECT D2_X_VEND VENDEDOR, 
			...
			SUM(CASE WHEN D2_X_TPOPE = 'BONIFICACAO' THEN D2_X_CRVEN ELSE 0 END) DEBITO
			FROM %table:SD2% SD2
		WHERE SD2.%notDel% AND D2_FILIAL = %xfilial:SD2% AND D2_X_CRVEN > 0 
		GROUP BY D2_X_VEND
		UNION
		SELECT D1_X_VEND VENDEDOR,
			SUM(CASE WHEN D1_X_TPOPE = 'DVBONIFICACAO' THEN D1_X_DBVEN ELSE 0 END) CREDITO,
			SUM(CASE WHEN D1_X_TPOPE = 'DVVENDAS' THEN D1_X_DBVEN ELSE 0 END) DEBITO
			FROM %table:SD1% SD1
		WHERE SD1.%notDel% AND D1_FILIAL = %xfilial:SD1% AND D1_X_DBVEN > 0
		GROUP BY D1_X_VEND) CRED_DEB
