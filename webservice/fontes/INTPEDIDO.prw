...
Manutenções										Autor													Data
Retirada dos bloqueios 						César Augusto Barbosa, Totvs Anikel			13/08/2013
...
Private lLiberAut 	:= .T.      //-- Se campo C6_QTDLIB será preenchido no processo de inclusão -- Gustavo Viana -- 18/02/2013
Private cCod      	:= ""
Private cLocal    	:= "01"
Private nQuant    	:= 0
	...
	If SZ3->(DbSeek(xFilial("SZ3") + cPedidoW))
		...
		Else
			...
			Else
				...
				lLiberAut := .T. //-- Gustavo Viana -- Restrições a liberação automática -- 18/02/2013
				...
				//-- Gustavo Viana -- Restrições a liberação automática -- 18/02/2013
				//-- Se tiver observção preenchida P.V. deverá ser analisado, ou seja, P.V. sem liberação automatica
				//If !Empty(Alltrim(SZ3->Z3_OBS))
				//	lLiberAut := .F.
				//EndIf
				//-- Se tiver condição de pagamento no parametro P.V. deverá ser analisado, ou seja, P.V. sem liberação automatica
				//If AllTrim(SZ3->Z3_CODPGTO) $ cCodPag
				//	lLiberAut := .F.
				//EndIf
				//-- Se Frete for diferente de CIF P.V. deverá ser analisado, ou seja, P.V. sem liberação automatica
				//If AllTrim(SZ3->Z3_FREPAGO) <> "C"
				//	lLiberAut := .F.
				//EndIf
				//-- Fim das Restrições -- 18/02/2013
		      ...
		      */
				...
				If SZ4->(DbSeek(xFilial("SZ4") + cPedidoW))
					While SZ4->(!Eof()) .And. 	SZ4->Z4_FILIAL == xFilial("SZ4") .And.;
						...
						//-- Se Tipo de operação web for BONIFICACAO P.V. deverá ser analisado, ou seja, P.V. sem liberação automatica -- Gustavo Viana -- 18/02/2013
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³INTPEDIDO ºAutor  ³Rodrigo Leite       º Data ³  23/07/13   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Valida o saldo em estoque dos pedidos digitados na WEB     º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP                                                         º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
