...
// criar parâmetro MV_TESDEV, com a TES que será usada na NF saída para troca
	...
	If (ParamIXB[1] == 3 .or. Inclui) .And. PARAMIXB[2] == 1 .and. dDataBase < Date()
		U_DispNFE()
	Endif
...
If SD1->(dbSeek(SF1->F1_FILIAL+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA))
	_cCorpoM += '<html>' 
	_cCorpoM += '<head>'
	_cCorpoM += '<title>Entrada de Nota com Data Retroativa</title>'
	...
	_cCorpoM += '</head>' 
	_cCorpoM += '<body>'
	_cCorpoM += '  <p align="center" class="style1"><strong>AVISO DE ENTRADA DE NFE COM DATA RETROATIVA</strong></p>' 
