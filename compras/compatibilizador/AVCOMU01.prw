...
//--------------------------------------------------------------------
/*/{Protheus.doc} AVCOMU01
Função de update de dicionários para compatibilização

@author TOTVS Protheus
@since  07/05/2014
@obs    Gerado por EXPORDIC - V.4.19.9.2 EFS / Upd. V.4.17.9 EFS
         Campos     --> Tabelas: SC7 (C7_X_WF, C7_X_WFID) e
		                        SCR (CR_X_WFID)
         Contempla as atualizações para:
          - Workflow de Aprovação do Pedido de Compra (U_AVCOMW10)
		...
		nos seguintes campos.
		   ...
		   SC7->C7_X_WF    => "3"		// Status 3 - Envio de e-mail de notificação Processado para comprador e fornecedor
@version 1.0
/*/
//--------------------------------------------------------------------
User Function AVCOMU01( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "AVCOMU01" )
				Else
					MsgStop( "Atualização não Realizada.", "AVCOMU01" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	'WF Status'																, ; //X3_TITSPA
	'WF Status'																, ; //X3_TITENG
	'Workflow Status'														, ; //X3_DESCRIC
	'Workflow Status'														, ; //X3_DESCSPA
	'Workflow Status'														, ; //X3_DESCENG
	''																		, ; //X3_PICTURE
	''																		, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Status do Workflow. Usado para controle' )
aAdd( aHlpPor, 'do status gerado pelo Workflow.' )
aHlpEng := {}
aHlpSpa := {}

PutHelp( "PC7_X_WF   ", aHlpPor, aHlpEng, aHlpSpa, .T. )
cTexto += "Atualizado o Help do campo " + "C7_X_WF" + CRLF

aHlpPor := {}
aAdd( aHlpPor, 'ID do Workflow. Usado para controle ID' )
aAdd( aHlpPor, 'do processo gerado pelo Workflow.' )
aHlpEng := {}
aHlpSpa := {}

PutHelp( "PC7_X_WFID ", aHlpPor, aHlpEng, aHlpSpa, .T. )
cTexto += "Atualizado o Help do campo " + "C7_X_WFID" + CRLF
...
PutHelp( "PCR_X_WFID ", aHlpPor, aHlpEng, aHlpSpa, .T. )
cTexto += "Atualizado o Help do campo " + "CR_X_WFID" + CRLF
