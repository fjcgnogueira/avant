...
Static Function GeraArqTRG()
	...
	BeginSql alias 'TRG'
		...
		SELECT F8_FILIAL, F8_NFDIFRE, F8_SEDIFRE, D1_BASEICM, D1_ICMSCOM, D1_VALICM, SUM(D1_TOTAL) AS TOTAL, D1_EMISSAO  
		FROM %table:SF8% AS SF8
		INNER JOIN %table:SD1% AS SD1 ON F8_FILIAL = D1_FILIAL AND F8_NFDIFRE = D1_DOC AND F8_SEDIFRE = D1_SERIE AND SD1.%notDel%
		...
		GROUP BY F8_FILIAL, F8_NFDIFRE, F8_SEDIFRE, D1_BASEICM, D1_ICMSCOM, D1_VALICM, D1_EMISSAO
		ORDER BY F8_NFDIFRE
