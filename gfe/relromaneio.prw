...
Static Function GeraArqTRB()
	...
	BeginSql alias "TRBGWN"
		...
		ISNULL(GWB_CDUNIT,'') AS GWB_CDUNIT, ISNULL(GWB_QTDE,'') AS GWB_QTDE, F2_VOLUME1
		...
		LEFT JOIN %table:GWB% AS GWB ON GW1_FILIAL = GWB_FILIAL AND GW1_NRDC = GWB_NRDC AND GW1_SERDC = GWB_SERDC AND GWB.%notDel%
		INNER JOIN %table:SF2% AS SF2 ON GW8_FILIAL = F2_FILIAL AND GW8_NRDC = F2_DOC AND GW8_SERDC = F2_SERIE AND SF2.%notDel%
		WHERE  GWN.%notDel% AND GWN_FILIAL = '010104' AND GWN_NRROM = %Exp:MV_PAR01%
