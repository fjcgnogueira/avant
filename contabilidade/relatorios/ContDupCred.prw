...
While !(cAliasCT2)->(Eof())
	...
	BeginSql alias cAliasDup
		...
		INNER JOIN %table:SE5% SE5 ON CT2_FILIAL = E5_FILIAL AND CV3_RECORI = SE5.R_E_C_N_O_ AND SE5.%notDel%
		WHERE CT2.%notDel% 
...
Static Function GerArqTRB1()
	...
	BeginSql alias 'TRB'
		...
		LEFT  JOIN %table:SE5% SE5 ON CT2_FILIAL = E5_FILIAL AND E5_DTDISPO BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)% AND CV3_RECORI = SE5.R_E_C_N_O_ AND SE5.%notDel%
		WHERE CT2.%notDel% 
			AND CT2_FILIAL = %xfilial:CT2% 
			AND CT2_DATA BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)% 
			AND CT2_CREDIT = '110503002' 
...
Static Function GerArqCT2()
	...
	BeginSql alias cAliasCT2
		...
		INNER JOIN SE5010 SE5 ON CT2_FILIAL = E5_FILIAL AND E5_DTDISPO BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)% AND CV3_RECORI = SE5.R_E_C_N_O_ AND SE5.%notDel%
		WHERE CT2.%notDel% 
			AND CT2_FILIAL = %xfilial:CT2%
			AND CT2_DATA BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)%
			AND CT2_CREDIT = '110503002'
