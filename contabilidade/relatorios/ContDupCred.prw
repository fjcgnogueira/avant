...
While !(cAliasCT2)->(Eof())
	...
	BeginSql alias cAliasDup
		...
		INNER JOIN %table:CV3% CV3 ON CT2_FILIAL = CV3_FILIAL AND CV3_TABORI = 'SE5' AND CV3_DTSEQ BETWEEN %Exp:DtoS(mv_par01-60)% AND %Exp:DtoS(mv_par02+60)% AND CT2.R_E_C_N_O_ = CV3_RECDES AND CV3.%notDel%
		INNER JOIN %table:SE5% SE5 ON CT2_FILIAL = E5_FILIAL AND E5_RECPAG = 'R' AND E5_SITUACA <> 'C' AND E5_TIPODOC NOT IN ('TR','DC','D2','JR','J2','MT','M2','DB','ES') AND CV3_RECORI = SE5.R_E_C_N_O_ AND SE5.%notDel%
		WHERE CT2.%notDel% 
...
Static Function GerArqTRB1()
	...
	BeginSql alias 'TRB'
		...
		SELECT CT2_DATA EMISSAO,CT2_HIST HISTORICO, CT2_VALOR VALOR FROM %table:CT2% CT2
		INNER JOIN %table:CV3% CV3 ON CT2_FILIAL = CV3_FILIAL AND CV3_TABORI = 'SE5' AND CV3_DTSEQ BETWEEN %Exp:DtoS(mv_par01-60)% AND %Exp:DtoS(mv_par02+60)%  AND CT2.R_E_C_N_O_ = CV3_RECDES AND CV3.%notDel%
		LEFT  JOIN %table:SE5% SE5 ON CT2_FILIAL = E5_FILIAL AND E5_RECPAG = 'R' AND E5_SITUACA <> 'C' AND E5_TIPODOC NOT IN ('TR','DC','D2','JR','J2','MT','M2','DB','ES') AND E5_DTDISPO BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)% AND CV3_RECORI = SE5.R_E_C_N_O_ AND SE5.%notDel%
		WHERE CT2.%notDel% 
			AND CT2_FILIAL = %xfilial:CT2% 
			AND CT2_DATA BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)% 
			AND CT2_CREDIT = '110503002' 
...
Static Function GerArqCT2()
	...
	BeginSql alias cAliasCT2
		...
		INNER JOIN CV3010 CV3 ON CT2_FILIAL = CV3_FILIAL AND CV3_TABORI = 'SE5' AND CV3_DTSEQ BETWEEN %Exp:DtoS(mv_par01-60)% AND %Exp:DtoS(mv_par02+60)% AND CT2.R_E_C_N_O_ = CV3_RECDES AND CV3.%notDel%
		INNER JOIN SE5010 SE5 ON CT2_FILIAL = E5_FILIAL AND E5_RECPAG = 'R' AND E5_SITUACA <> 'C' AND E5_TIPODOC NOT IN ('TR','DC','D2','JR','J2','MT','M2','DB','ES') AND E5_DTDISPO BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)% AND CV3_RECORI = SE5.R_E_C_N_O_ AND SE5.%notDel%
		WHERE CT2.%notDel% 
			AND CT2_FILIAL = %xfilial:CT2%
			AND CT2_DATA BETWEEN %Exp:DtoS(mv_par01)% AND %Exp:DtoS(mv_par02)%
			AND CT2_CREDIT = '110503002'
...
Static Function AjustaSX1(cPerg)
	...
	aHelpPor := {"Relat๓rio:","- Cr้dito a Mais","- Cr้dito Duplicado"}
	PutSX1(cPerg,"03","Relat๓rio ?"    ,"","","mv_ch3","N",1,0,1,"C","NaoVazio","","","","mv_par03","Cred A Mais","Cred A Mais","Cred A Mais","","Cred Duplicado","Cred Duplicado","Cred Duplicado","","","","","","","","","",aHelpPor,aHelpEng,aHelpSpa)
